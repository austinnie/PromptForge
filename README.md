...
PromptForge 是一个基于 Stable Diffusion 与多家 AI API 的智能对话式创作工具，也是一套覆盖「生成 → 加工 → 排版 → 分发」的全链路 AI 内容创作引擎。用户通过自然语言描述即可生成高质量图片、视频，甚至完成从小说脚本到成片的全自动多媒体创作，支持本地模型与云端 API 双模式。
...
### 🎨 图像生成
- **双模式支持**：本地 Stable Diffusion 模型 + 云端 API
- **文生图**：输入文字描述生成图片
- **图生图**：基于参考图生成新图片
- **多引擎支持**：Agnes / Pollinations / HuggingFace / Stability / Replicate / Free API
- **自动回退**：API 失败时自动切换到备用引擎
- **多图合成**：2 人双人合成，3-6 人多人群像合成

### 🎬 视频生成
- **文生视频**：输入文字描述生成视频
- **长视频拼接**：自动拆分多段生成后拼接成长视频（支持 4-12 秒/段）
- **参考图生成**：支持上传参考图生成视频
- **支持多引擎**：Agnes Video 2.5 Flash

### 🎵 音乐生成
- **AI 音乐大师**：自动写词、谱曲
- **双模式引擎**：MIDI 合成 / MusicGen（Meta）
- **多情绪支持**：宁静 / 深情 / 欢乐 / 壮丽 / 神秘 / 浪漫 / 活力
- **智能硬件检测**：根据 GPU 自动选择最优模式
