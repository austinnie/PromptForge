...
class NewsAggregator:
    ...
    def _generate_ai_summary(self, articles: List[Dict]) -> str:
        ...
        LLM 后端优先级：
          1. settings.news_llm_backends（NEWS_LLM_BACKENDS 环境变量）
          2. settings.llm_backends（LLM_BACKENDS 环境变量）
          3. LLMClient 硬编码默认值
        """
        if not articles:
            return "暂无新闻"
...
请生成每日新闻简报摘要："""
            ...
            # ✅ 走 settings，而非直接读 env
            overrides = dict(
                temperature=temperature,
                max_tokens=2048,
                timeout=timeout,
            )
            if settings.news_llm_backends:
                overrides["backends"] = settings.news_llm_backends
